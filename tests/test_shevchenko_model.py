...
def test_Shev_other_column_names(carbognani2019, idc, alphac, magc):

    expected = pyedra.Shev_fit(carbognani2019).model_df

    carbognani2019.columns = [idc, alphac, magc]
    result = pyedra.Shev_fit(
...
@pytest.mark.parametrize("magc", ["v", "i"])
@check_figures_equal()
def test_plot_Shev_fit_other_column_names(
